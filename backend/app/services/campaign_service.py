...
class ContactService:
    ...
    def process_csv(db: Session, file_content: bytes, list_name: str):
        ...
        for encoding in encodings:
            ...
            for sep in separators:
                try:
                    ...
                    # 调试日志
                    print(
                        f"尝试读取 {encoding} | {sep} -> 列名: {list(temp_df.columns)}"
                    )
                ...
                for enc in ["utf-16", "utf-8-sig", "gb18030"]:
                    try:
                        ...
                        if "\t" in text_content:
                            ...
                            if len(df.columns) > 1 and any(
                                "email" in str(c).lower() for c in df.columns
                            ):
        ...
        if df is None:
            raise ValueError(
                f"无法解析文件。请确保文件包含 EmailAddr 列。尝试的格式均失败。"
            )
        ...
        if final_target_col:
            ...
            raise ValueError(
                f"最终未能定位 EmailAddr 列。检测到的列: {list(df.columns)}"
            )
        ...
        for index, row in df.iterrows():
            # 提取所有变量
            extra_vars = {
                k: str(row[k]) if pd.notna(row[k]) else "" for k in var_columns
            }
            ...
            name_key = next(
                (k for k in extra_vars.keys() if k.lower() == "username"), None
            )
            if not name_key:
                name_key = next(
                    (k for k in extra_vars.keys() if k.lower() == "name"), None
                )
